...
import {
  Step, Workflow, StepType, WorkflowInternal,
  SimpleStepInternal, WorkflowStepInternal,
  StepInternal, WorkflowOutput, Binding, StepOutput, StepExitAction
} from './types'
...
export class WorkflowEngine {
  ...
  private populateSimpleStep (step: SimpleStepInternal) {
    ...
    if (step.externalWorkflow) {
      const externalWorkflow = WorkflowUtils.createFromFilePath(join(this.rootPath, step.externalWorkflow.path))
  ...
  private prepareBindings (workflow: WorkflowInternal) {
    workflow.bindingsInternal = Object.assign({}, commonBindings, this.extractBindings(workflow.bindings))
  ...
  shouldSkipStep (step: StepInternal, input: any, bindings: Record<string, any> = {}): boolean {
    ...
    return !!step.conditionExpression &&
      !step.conditionExpression.evaluate(input, allBindings)
  }
  ...
  private async executeSimpleStep (step: SimpleStepInternal, input: any, bindings: Record<string, any> = {}): Promise<StepOutput> {
    if (this.shouldSkipStep(step, input, bindings)) {
      return { skipped: true }
    }
    bindings.context = bindings.context || {}
    const allBindings = Object.assign({}, this.workflow.bindingsInternal, bindings, this.getContextFunctions(bindings.context))
  ...
  private async executeWorkflowStep (workflowStep: WorkflowStepInternal, input: any, bindings: Record<string, any> = {}): Promise<StepOutput> {
  ...
  private executeStepIteration (step: StepInternal, input: any, bindings: Record<string, any> = {}): Promise<StepOutput> {
    switch (step.type) {
  ...
  private async executeStepInternal (step: StepInternal, input: any, bindings: Record<string, any> = {}): Promise<StepOutput> {
    let stepInput = input
    ...
    if (step.loopOverInput) {
      ...
      const output = await Promise.all(stepInput.map(async (inputElement) => {
        try {
  ...
  executeStep (step: StepInternal, input: any, bindings: Record<string, any> = {}): Promise<StepOutput> {
  ...
  handleError (error: any, stepName: string): WorkflowOutput {
    const status = WorkflowUtils.isAssertError(error) ? 400 : error.response?.status || error.status || 500
